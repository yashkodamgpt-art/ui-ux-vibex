...
const MainApp: React.FC<MainAppProps> = ({ user, onLogout, onProfileUpdate }) => {
  ...
  const handleSaveTag = (tagData: Omit<Tag, 'id' | 'memberIds'>) => { if (editingTag) { setTags(prevTags => prevTags.map(t => t.id === editingTag.id ? { ...t, ...tagData } : t)); } else { const newTag: Tag = { ...tagData, id: `tag-${Date.now()}`, memberIds: [] }; setTags(prevTags => [...prevTags, newTag]); } setIsCreateTagModalOpen(false); setEditingTag(null); };
  const handleDeleteTag = (tagId: string) => { setConfirmation({ title: "Delete Tag?", message: "Are you sure? This action cannot be undone.", onConfirm: () => { setTags(prevTags => prevTags.filter(t => t.id !== tagId)); setConfirmation(null); } }); };
  const handleOpenAssignTagModal = (friend: Friend) => { setAssigningFriend(friend); setIsAssignTagModalOpen(true); };
  const handleSaveFriendTags = (friendId: string, selectedTagIds: string[]) => { setTags(prevTags => prevTags.map(tag => { const hasFriend = tag.memberIds.includes(friendId); const shouldHaveFriend = selectedTagIds.includes(tag.id); if (hasFriend && !shouldHaveFriend) return { ...tag, memberIds: tag.memberIds.filter(id => id !== friendId) }; if (!hasFriend && shouldHaveFriend) return { ...tag, memberIds: [...tag.memberIds, friendId] }; return tag; })); setIsAssignTagModalOpen(false); setAssigningFriend(null); };
  const handleRemoveFriend = (friendId: string) => { const friendToRemove = friends.find(f => f.id === friendId); if (!friendToRemove) return; setConfirmation({ title: `Remove ${friendToRemove.username}?`, message: `This will remove them from all your tags.`, onConfirm: () => { setFriends(prev => prev.filter(f => f.id !== friendId)); setTags(prev => prev.map(tag => ({ ...tag, memberIds: tag.memberIds.filter(id => id !== friendId) }))); setConfirmation(null); } }); };
  const handleSendRequest = (toUserId: string) => { setFriendRequests(prev => [...prev, { fromUserId: user.id, toUserId }]); };
  const handleAcceptRequest = (fromUserId: string) => { const userToAdd = MOCK_USERS_DATABASE.find(u => u.id === fromUserId); if (userToAdd) { setFriends(prev => [...prev, userToAdd]); } setFriendRequests(prev => prev.filter(req => !(req.fromUserId === fromUserId && req.toUserId === user.id))); };
