...
const MainApp: React.FC<MainAppProps> = ({ user, onLogout, onProfileUpdate }) => {
  ...
  const visibleSessions = useMemo(() => {
    ...
    tags.forEach(tag => {
        if (tag.memberIds.includes(user.id)) {
            userTagIds.add(tag.id);
        }
    });

    return sessions.filter(session => {
        if (session.privacy !== 'private') return true;
        if (session.creator_id === user.id) return true;
        if (session.visibleToTags) {
  ...
  const handleCancelCreate = useCallback(() => {
    setIsCreateMenuOpen(false); setIsPlacementMode(false); setSelectedSessionType(null);
    setIsCreateModalOpen(false); setNewEventCoords(null);
  }, []);
  const handleCreateButtonClick = () => {
    if (isCreateMenuOpen || isPlacementMode) { handleCancelCreate(); } else { setIsCreateMenuOpen(true); }
  };
  const handleSelectSessionType = (type: SessionType) => {
    setSelectedSessionType(type); setIsPlacementMode(true); setIsCreateMenuOpen(false);
  };
  const handleMapPlacement = (coords: { lat: number; lng: number }) => {
    if (activeVibe) { alert("You are already in a Vibe."); handleCancelCreate(); return; }
    setNewEventCoords(coords); setIsCreateModalOpen(true); setIsPlacementMode(false);
  };
  const handleCreateEvent = async (eventData: Omit<Session, 'id' | 'creator' | 'creator_id' | 'lat' | 'lng' | 'participants' | 'creator'>) => {
    ...
    const newSession: Session = {
      ...eventData,
      id: Math.floor(Math.random() * 10000), lat: newEventCoords.lat, lng: newEventCoords.lng,
      creator_id: user.id, participants: [user.id], creator: { username: user.profile.username },
      sessionType: selectedSessionType || 'vibe',
      creatorGender: user.profile.gender, // NEW
    };
  ...
  const handleCloseEvent = async (sessionId: number) => {
    ...
    if (activeVibe?.id === sessionId) { setActiveVibe(null); setIsChatVisible(false); }
  };
  const handleExtendEvent = async (sessionId: number, minutes: number) => {
      setSessions(prev => prev.map(s => s.id === sessionId ? { ...s, duration: s.duration + minutes } : s));
  };
  const handleJoinVibe = async (sessionId: number) => {
    ...
    setSessions(prev => prev.map(s => {
      if (s.id === sessionId) { const newParticipants = [...s.participants, user.id]; joinedSession = { ...s, participants: newParticipants }; return joinedSession; }
      return s;
    }));
  ...
  const handleSendMessage = async (text: string) => {
      ...
      const newMessage: SessionMessage = {
        id: Math.floor(Math.random() * 10000), sender_id: user.id, session_id: activeVibe.id,
        text: text, created_at: new Date().toISOString(), sender: { username: user.profile.username },
      };
  ...
  const handleViewFriendProfile = (friend: Friend) => {
      const userToView: User = {
        id: friend.id, email: `${friend.username.toLowerCase()}@campus.dev`,
        profile: {
          username: friend.username, bio: `A ${friend.branch} student graduating in ${friend.year}.`,
          branch: friend.branch, year: friend.year, expertise: [], interests: [],
          cookieScore: friend.cookieScore, privacy: 'public', skillScores: {}, vouchHistory: [],
          gender: friend.gender, // NEW
        }
      };
      ...
      <main className="flex-grow relative overflow-hidden">
        {error && (<div className="fixed top-4 left-1/2 -translate-x-1/2 z-[2000] bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded shadow-lg max-w-md w-11/12" role="alert">{/* ... */}</div>)}
          <div className={`h-full w-full ${activeTab === 'Home' ? 'block' : 'hidden'}`}>
            <MapView ref={mapViewRef} isVisible={activeTab === 'Home'} isCreateMode={isPlacementMode} userLocation={userLocation} onSetUserLocation={setUserLocation} onMapClick={handleMapPlacement} events={filteredSessions} user={user} activeVibe={activeVibe} onCloseEvent={handleCloseEvent} onExtendEvent={handleExtendEvent} onJoinVibe={handleJoinVibe} onViewChat={() => setIsChatVisible(true)} activeFilter={activeFilter} campusZones={campusZones} />
