...
const CreateEventModal: React.FC<CreateEventModalProps> = ({ isOpen, onClose, onSubmit, sessionType, tags, friends, user }) => {
    ...
    useEffect(() => {
        if (isOpen && sessionType) {
            ...
            setDuration(60); setError(''); setSelectedEmoji(sessionConfigs[sessionType].emoji);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onSubmit({
            ...
            participantRoles: { [user.id]: sessionType === 'seek' ? 'seeking' : 'offering' },
        });
    };
                ...
                <form onSubmit={handleSubmit} className="w-full max-w-lg bg-white rounded-2xl shadow-2xl p-6 sm:p-8 space-y-4 transform transition-all duration-300 scale-100 max-h-[90vh] flex flex-col">
                    ...
                    <div className="space-y-4 overflow-y-auto pr-2 flex-grow">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-3 mt-1"><span className="text-3xl">{selectedEmoji}</span><input id="title" type="text" value={title} onChange={e => setTitle(e.target.value)} required className={`block w-full px-4 py-2 bg-gray-50 border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:ring-2 ${colors.ring}`} placeholder={config.placeholder} /></div>
                        </div>
                        ...
                        {sessionType === 'seek' && (
                            <div className="p-3 bg-gray-50 rounded-lg">
                                <label htmlFor="helpCategory" className="text-sm font-medium text-gray-700 mb-2 block">Help Category</label>
                                <select id="helpCategory" value={helpCategory} onChange={e => setHelpCategory(e.target.value as any)} className={`block w-full px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:ring-2 ${colors.ring}`}>
                                    <option>General</option><option>Academic</option><option>Project</option><option>Tech</option>
                                </select>
                            </div>
                        )}
                        {sessionType === 'cookie' && (
                             <div className="p-3 bg-gray-50 rounded-lg space-y-3">
                                <div><label htmlFor="skillTag" className="text-sm font-medium text-gray-700 mb-2 block">Skill Tag</label><select id="skillTag" value={skillTag} onChange={e => setSkillTag(e.target.value)} className={`block w-full px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:ring-2 ${colors.ring}`}>{user.profile.expertise.length > 0 ? user.profile.expertise.map(skill => <option key={skill}>{skill}</option>) : <option disabled>Add skills in your profile!</option>}</select></div>
                                <div><label htmlFor="expectedOutcome" className="text-sm font-medium text-gray-700">Expected Outcome (Optional)</label><input id="expectedOutcome" type="text" value={expectedOutcome} onChange={e => setExpectedOutcome(e.target.value)} className={`mt-1 block w-full px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:ring-2 ${colors.ring}`} placeholder="e.g., You'll be able to build a simple web page" /></div>
                             </div>
                        )}
