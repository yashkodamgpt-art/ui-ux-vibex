...
const SkeletonCard: React.FC = () => (
    <div className="p-4 flex items-center space-x-4 rounded-xl bg-white animate-pulse">
...
const MessagesPanel: React.FC<MessagesPanelProps> = ({ conversations, currentUser, friends, onOpenConversation, isLoading }) => {
  ...
  return (
    <div className={`p-4 space-y-3 h-full ${isEmpty ? 'bg-gradient-to-b from-gray-50 to-gray-100 flex items-center justify-center' : ''}`}>
      {isLoading ? (
        ...
        <div className="text-center">
          ...
          <h3 className="text-xl font-bold text-gray-800">Your inbox is quiet</h3>
          <p className="text-gray-500 mt-2 max-w-xs mx-auto">Send a message from a friend's profile to start a conversation.</p>
        </div>
      )}
    </div>
  );
