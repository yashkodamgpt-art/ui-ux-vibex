...
const VibeChatPanel: React.FC<VibeChatPanelProps> = ({ isOpen, onClose, vibe, messages, user, onSendMessage, onLeaveVibe, onViewProfile }) => {
    ...
    const handleFormSubmit = (e: React.FormEvent) => { e.preventDefault(); if (messageText.trim()) { onSendMessage(messageText); setMessageText(''); } };
        ...
        <>
            ...
            <div className="flex-shrink-0 pt-2">
                <div className="flex flex-wrap items-center justify-center gap-2 mb-3">
                    {QUICK_REPLIES.map(reply => ( <button key={reply} onClick={() => onSendMessage(reply)} className="px-4 py-2 text-sm font-medium rounded-full bg-purple-100 text-purple-800 hover:bg-purple-200 transition-colors">{reply}</button>))}
                </div>
                 <form onSubmit={handleFormSubmit} className="flex items-center gap-2 p-2">
                    <input type="text" value={messageText} onChange={e => setMessageText(e.target.value)} placeholder="Type your message..." className="flex-grow px-4 py-2 bg-white border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-purple-500" />
                        ...
                        <li key={participant.id}>
                            <button onClick={() => onViewProfile(participant.username)} className="w-full text-left flex items-center justify-between p-3 bg-white rounded-lg shadow-sm hover:bg-gray-100 transition-colors">
                                <div className="flex flex-col">
                                    ...
                                    {role && <span className={`text-xs font-semibold mt-1 px-2 py-0.5 rounded-full self-start ${role === 'seeking' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>{role.charAt(0).toUpperCase() + role.slice(1)}</span>}
                                </div>
        ...
        <>
            ...
            <div className={`fixed bottom-0 left-0 right-0 z-[2010] bg-gray-50 rounded-t-2xl shadow-2xl transition-transform duration-300 ease-in-out ${isOpen ? 'translate-y-0' : 'translate-y-full'}`} style={{ height: '75vh' }} role="dialog" aria-modal="true" aria-labelledby="vibe-chat-title">
                <div className="p-4 flex flex-col h-full">
                    ...
                    <div className="flex-shrink-0 pt-2 border-t border-gray-200"> {!isCreator && ( <button onClick={() => onLeaveVibe(vibe.id)} className="w-full mt-2 py-3 bg-red-500 text-white font-bold rounded-lg hover:bg-red-600 transition-colors">Leave Vibe</button> )} </div>
                </div>
            </div>
        </>
    );
