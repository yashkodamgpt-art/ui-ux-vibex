        ...
        <div className="relative" ref={menuRef}>
            ...
            {isOpen && (
                <div className="absolute right-0 bottom-full mb-2 w-48 bg-white rounded-md shadow-lg z-20 origin-bottom-right">
                    <div className="py-1">
                        <button onClick={() => { onMakeLeader(); setIsOpen(false); }} className="w-full text-left flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
        ...
        <div className="flex-grow overflow-y-auto py-4 px-2">
            <ul className="space-y-2">
                {participants.map(participant => {
                    ...
                    const roleColor = role === 'seeking' || role === 'giver' ? (role === 'seeking' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800') : null;
                    const canTransfer = isCreator && participant.id !== user.id && vibe.privacy === 'public';

                    return (
                        <li key={participant.id}>
                            <div className="w-full flex items-center justify-between p-3 bg-white rounded-lg shadow-sm">
